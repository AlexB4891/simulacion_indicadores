...
porcentaje_declarado <- list(
  tabla_trabajo %>% 
    filter(dummy_aps_declarado_101 == 1,
           dummy_revisar == 0) %>% 
    select(anio_fiscal,identificacion_informante_anon,net_profit_ratio) %>% 
...
tabla_estadisticas <- porcentaje_declarado %>% 
  ...
  mutate(net_profit_ratio_no_cero = if_else(net_profit_ratio == 0,NA_real_,net_profit_ratio)) %>% 
  group_by(panel,anio_fiscal) %>% 
  summarise( `Obs` = n(),
             `Vacios` = sum(is.na(net_profit_ratio)),
             `Cero's` = sum(net_profit_ratio == 0),
             `Mín` = min(net_profit_ratio_no_cero,na.rm = T),
             Median = median(net_profit_ratio_no_cero,na.rm = T),
             Max = max(net_profit_ratio_no_cero,na.rm = T),
             Media = mean(net_profit_ratio_no_cero,na.rm = T),
             SD = sd(net_profit_ratio_no_cero,na.rm = T) ) %>% 
  mutate(across(c( `Mín`,
                   Median,
                   Max,
                   Media,
...
tabla_distribucion <- porcentaje_declarado %>% 
  ...
  summarise(net_profit_ratio_1214 = mean(net_profit_ratio_no_cero,na.rm = T))

tabla_distribucion <- tabla_distribucion %>% 
  ungroup() %>% 
  group_by(panel) %>% 
  mutate(mean = mean(net_profit_ratio_1214,na.rm = T),
         sd = sd(net_profit_ratio_1214,na.rm = T),
         median = median(net_profit_ratio_1214,na.rm = T),
         upper = mean + sd,
         lower = mean - sd) %>% 
  rowwise() %>% 
  mutate(dummy = between(net_profit_ratio_1214,lower,upper))

grafico_distribucion <- tabla_distribucion %>% 
  filter(!is.nan(net_profit_ratio_1214)) %>% 
  ggplot(mapping = aes(x = net_profit_ratio_1214,color = panel,fill = panel)) +
