...
tabla_estadisticas <- porcentaje_declarado %>% 
  ...
  summarise(
    `Obs` = n(),
    `Vacios` = sum(is.na(beneficiarios_ext)),
    `Cero's` = sum(beneficiarios_ext == 0),
    `Mín` = min(beneficiarios_ext_no_cero,na.rm = T),
    Median = median(beneficiarios_ext_no_cero,na.rm = T),
    Max = max(beneficiarios_ext_no_cero,na.rm = T),
    Media = mean(beneficiarios_ext_no_cero,na.rm = T),
    SD = sd(beneficiarios_ext_no_cero,na.rm = T)
  ) %>% 
  mutate(across(c(
    `Mín`,
    Median,
