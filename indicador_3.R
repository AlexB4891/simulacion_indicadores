...
tabla_trabajo <- tabla_trabajo %>% 
  mutate(concentracion = beneficiarios_pff/total_benef_final)
...
porcentaje_declarado <- list(
  tabla_trabajo %>% 
    filter(dummy_aps_declarado_101 == 1,
           dummy_revisar == 0) %>% 
    select(anio_fiscal,identificacion_informante_anon,concentracion) %>% 
...
tabla_estadisticas <- porcentaje_declarado %>% 
  ...
  mutate(concentracion = if_else(concentracion == 0,NA_real_,concentracion)) %>% 
  group_by(panel,anio_fiscal) %>% 
  summarise(
    `Obs` = n(),
    `Vacios` = sum(is.na(concentracion)),
    `Cero's` = sum(concentracion == 0),
    `Mín` = min(concentracion,na.rm = T),
    Median = median(concentracion,na.rm = T),
    Max = max(concentracion,na.rm = T),
    Media = mean(concentracion,na.rm = T),
    SD = sd(concentracion,na.rm = T)
  ) %>% 
  mutate(across(c(
    `Mín`,
    Median,
...
tabla_distribucion <- porcentaje_declarado %>% 
  ...
  summarise(conc_14 = mean(concentracion,na.rm = T))

tabla_distribucion <- tabla_distribucion %>% 
  ungroup() %>% 
  group_by(panel) %>% 
  mutate(mean = mean(conc_14,na.rm = T),
         sd = sd(conc_14,na.rm = T),
         median = median(conc_14,na.rm = T),
         upper = mean + sd,
         lower = mean - sd) %>% 
  rowwise() %>% 
  mutate(dummy = between(conc_14,lower,upper))

grafico_distribucion <- tabla_distribucion %>% 
  filter(!is.nan(conc_14)) %>% 
  ggplot(mapping = aes(x = conc_14,color = panel,fill = panel)) +
