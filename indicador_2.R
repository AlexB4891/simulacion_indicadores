...
tabla_estadisticas <- porcentaje_declarado %>% 
  ...
  mutate(pocent_pff_no_cero = if_else(porcentaje_pff == 0,NA_real_,porcentaje_pff)) %>% 
  group_by(panel,anio_fiscal) %>% 
  summarise(
    `Obs` = n(),
    `Vacios` = sum(is.na(porcentaje_pff)),
    `Cero's` = sum(porcentaje_pff == 0),
    `Mín` = min(pocent_pff_no_cero,na.rm = T),
    Median = median(pocent_pff_no_cero,na.rm = T),
    Max = max(pocent_pff_no_cero,na.rm = T),
    Media = mean(pocent_pff_no_cero,na.rm = T),
    SD = sd(pocent_pff_no_cero,na.rm = T)
  ) %>% 
  mutate(across(c(
    `Mín`,
    Median,
...
tabla_distribucion <- porcentaje_declarado %>% 
  ...
  summarise(pf_14 = mean(pocent_pff_no_cero,na.rm = T))

tabla_distribucion <- tabla_distribucion %>% 
  ungroup() %>% 
  group_by(panel) %>% 
  mutate(mean = mean(pf_14,na.rm = T),
         sd = sd(pf_14,na.rm = T),
         median = median(pf_14,na.rm = T),
         upper = mean + sd,
         lower = mean - sd) %>% 
  rowwise() %>% 
  mutate(dummy = between(pf_14,lower,upper))

grafico_distribucion <- tabla_distribucion %>% 
  filter(!is.nan(pf_14)) %>% 
  ggplot(mapping = aes(x = pf_14,color = panel,fill = panel)) +
